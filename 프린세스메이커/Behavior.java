...
//        }
    ...
    public static void read(Character character) {
        ...
        // 2-1. 동물농장
        if (book == 1) {
            System.out.println(character.getName() + " 공주는 동물농장 책을 꺼내들었다.");
            System.out.println();
            ...
            System.out.println(character.getName() + " 공주: (흠.. 동물들의 반란이라.. 약간 소름 끼치는걸)");
        ...
        // 2-2. 자신있게 말하기
            ...
            System.out.println(character.getName() + " 공주는 자신있게 말하기 책을 꺼내들었다.");
            System.out.println();

            System.out.println("이론과 실제 경험이 조화로운 책으로 혼자서 할 수 있도록 도와주는 지침서입니다. " +
                    "말하기를 어떻게 훈련할지 자세히 소개하여 혼자서도 충분히 연습할 수 있습니다. " +
                    "실험을 통해 검증받은 내용으로 어린이부터 청소년까지 모두 활용할 수 있습니다. " +
            ...
            System.out.println(character.getName() + " 공주는 마더 테레사 위인전 책을 꺼내들었다.");
            System.out.println();

            System.out.println(character.getName() + " 공주: (내가 정말 존경하는 분이야.. 여러 번 읽어야겠다.)");
            System.out.println();

            System.out.println("책을 펼쳐 한줄 한줄 집중하면서 읽기 시작했다.");
            System.out.println();

            System.out.println("오늘날 가장 큰 병은 결핵이나 나병이 아니라 다른 사람으로부터 사랑받지 못하고 남이 필요로 하지도 않으며 " +
                    "남으로부터 보살핌을 받지 못하는 것입니다.");
            System.out.println("그대가 판단과 행동에 혼란을 느낄 때에는 그것이 가장 가난한 사람의 입장에 서 있는가를 먼저 생각하라.");
            System.out.println();
        }

        System.out.println("-- 책을 모두 읽었습니다. --");
        System.out.println(character.getName() + " 공주: (역시 책을 읽으면 머리가 똑똑해지는 기분이야! 이 느낌을 기억하도록 독후감을 쓸까?)");
        System.out.print("독후감을 쓰시겠습니까? yes/no");
        String write_essay = in.next();

        // 독후감
        if (write_essay.equals("yes")) { // Objects 객체명 수정하기
            System.out.println(character.getName() + " 공주: (역시 이 느낌을 기억하도록 독후감을 쓰는게 낫겠어!)");
            System.out.println();
            ...
            System.out.println(character.getName() + " 공주: (음.. 그냥 다음에 쓰자.)");
        }
    }
    ...
    public static void exercise(Character character) {
        ...
        if (routine == 1) {
            ...
            System.out.println(character.getName() + " 공주: 흐어억.. 더 이상은...");
            ...
            System.out.println(character.getName() + " 공주: (윽..ㅡㅡ)");
        }
        else if (routine == 2) {
            ...
            System.out.println(character.getName() + " 공주: 으쌰 으쌰");
            System.out.println();
